...
data_model = (function() {
    ...
    return {
        ...
        funcaoparatestar: function() {
            //Função para testar se os dados estão ok!
            console.log(dados);
        }
    }
})();
...
controller = function(ui, data) {
    let adicionarItem = function() {
        ...
        if (input.descricao !== "" && !isNaN(input.valor) && input.valor > 0) {
            ...
            //3.1 Limpa os dados
            ui.limpaOsDados();
            //4.0 Atualizar os totais
            atualizaTotais();

        } else {
            alert("Você deve preencher os campos de descrição e valor (valor númerico e positivo)");
        }
    };
    ...
    let configuraOsListeners = function() {
        ...
        document.querySelector(DOMStrings.btnSalvar).addEventListener('click', function(event) {
            adicionarItem();
